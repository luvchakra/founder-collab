// Entry point for every /api/v1/* request against module-gst's resources -- same shape
// as module-inventory/api-v1/router.ts, module-fsm/api-v1/router.ts, and
// module-crm/api-v1/router.ts.
...
import { handle as einvoices } from "./resources/einvoices.server";
...
export const RESOURCES: Record<string, ResourceHandler> = { einvoices };
...
export async function handleApiV1Request(
  ...
  try {
    ...
    if (licenseError) throw new ApiError(500, "internal_error", "Could not verify the gst license.");
    if (!licensed) {
      throw new ApiError(403, "module_not_licensed", "This business does not have an active gst (Compliance) license.");
    }
