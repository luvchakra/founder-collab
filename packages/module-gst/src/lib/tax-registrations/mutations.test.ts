...
import { shouldMirrorToBusinessSettings } from "./mutations";
...
describe("shouldMirrorToBusinessSettings", () => {
  ...
  it("is case-sensitive -- catalog codes are always upper-cased before storage, so a lowercase mismatch is a bug to surface, not silently accept", () => {
    expect(shouldMirrorToBusinessSettings("in", "gst")).toBe(false);
  });
});
