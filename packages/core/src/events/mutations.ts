 ...
 * calling user through the normal RLS-scoped client by default, same as any other
 * write -- a publisher normally always has a real business_id it's already a member of.
 *
 * `client` (CRM-07.11): the one exception is a session-less webhook capturing a domain
 * event on a business's behalf with no logged-in user to authorize it (e.g. auto-created
 * lead capture) -- same optional-override DI shape `interactions/matching.ts#CrmClientOverrides`
 * and `parties/mutations.ts#createParty()` already use elsewhere in this codebase.
 *
