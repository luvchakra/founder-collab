// Hand-written rather than the generic CRUD factory: creating a purchase order also
// means creating its line items, and po_number needs generating -- both beyond what a
// single-table factory expresses.
//
// v1 is create-only (no PATCH): the workflow actions that mutate an existing PO
// (approve/send/receive) have real side effects (stock movements, audit rows) best
// exposed as their own endpoints once a real integration needs to drive them, not as a
// generic PATCH that could smuggle a status change past those rules. Same call as the
// original stockpilot-ai-ops resource this was promoted from, and as sales-orders.server.ts.
...
const LIST_SELECT =
  "id, po_number, status, order_date, expected_delivery_date, supplier_id, warehouse_id, subtotal, discount_amount, tax_amount, cgst_amount, sgst_amount, igst_amount, shipping_amount, total_amount, notes, created_at, updated_at";
...
async function fetchWithItems(db: ReturnType<typeof createAdminClient>, id: string) {
  const { data: po, error } = await db.from("purchase_orders").select(LIST_SELECT).eq("id", id).single();
  if (error) throw new ApiError(500, "internal_error", error.message);
  const { data: items, error: itemsError } = await db
    .from("purchase_order_items")
    .select("id, product_id, quantity, received_quantity, unit_cost, tax_rate, cgst_amount, sgst_amount, igst_amount")
...
export async function handle(
  ...
  if (request.method === "GET" && id) {
    const { data: po, error } = await db
      ...
      .maybeSingle();
    if (error) throw new ApiError(500, "internal_error", error.message);
    if (!po) throw new ApiError(404, "not_found", "No purchase order found with that id.");
  ...
  if (request.method === "POST" && !id) {
    ...
    const { data: po, error } = await db
      ...
      .insert({
        ...
        expected_delivery_date: typeof body["expected_delivery_date"] === "string" ? body["expected_delivery_date"] : null,
    ...
    if (items.length > 0) {
      const { error: itemsError } = await db.from("purchase_order_items").insert(
        items.map((item) => ({ org_id: ctx.businessId, purchase_order_id: po.id, ...item })),
      );
      if (itemsError) throw new ApiError(400, "invalid_request", itemsError.message);
    }

    return jsonResponse({ data: await fetchWithItems(db, po.id) }, 201);
  }
