...
| Supabase advisor findings | **Investigated and mostly fixed 2026-09-09.** ~~47 `function_search_path_mutable` warnings on `inventory`'s compat-view triggers/`next_*_number()` helpers~~ — fixed via `alter function ... set search_path` (no body changes; every one already fully-qualifies its own references, confirmed before touching). ~~`public.rls_auto_enable()` (a Supabase-project-default event trigger, not ours) callable by `anon`/`authenticated`~~ — fixed in two passes (`20260909030000`, then `20260909040000` correcting the first attempt's `revoke ... from anon, authenticated` no-op against what turned out to be a `PUBLIC` grant). Also dropped two genuinely dead tables found along the way (`inventory.demo_seed_batches`/`records`, superseded scaffolding never wired to anything, see `20260909020000`). **Left alone, confirmed intentional, not a gap:** the remaining `rls_enabled_no_policy` findings (`core.api_rate_limit_counters`, `core.demo_seed_batches`/`records`, `core.number_sequences`, `discovery.interest_signups`) — each one's own creating migration already documents zero-client-policy as the deliberate design (a SECURITY DEFINER function or service-role-only path is the sole access route). **Follow-up 2026-09-09: `unindexed_foreign_keys` and `multiple_permissive_policies` also
fixed.** ~~29 `unindexed_foreign_keys`~~ — one covering b-tree index per flagged FK column,
split into 4 migrations by owning schema (`20260909050000` core, `...050100` crm,
`...050200` fsm, `...050300` inventory) rather than one cross-schema file, matching
00-MASTER-PLAN.md §4's schema-ownership rule even though `lint-migration-schema.mjs`'s
own regex only checks `create table`/`alter table`, not indexes. Plain `create index`
(not `concurrently`) — every table involved is empty or near-empty in dev, no
long-lock risk; a production rollout of the same set might prefer `concurrently`.
~~5 `multiple_permissive_policies` (`core.user_profiles`, two SELECT policies)~~ — merged
into one policy or'ing both conditions together (`20260909050400`), same visible rows,
one policy evaluation instead of two; dry-run verified in a rolled-back transaction
before applying for real. Re-ran `get_advisors(type: performance)` after all 5
migrations: both findings are gone entirely. **Left alone, as before:** 42→71
`unused_index` (the 29 new ones are trivially "unused" in a dev project with no real
traffic yet, not a new problem; judging which of the original 42 are safe to drop from
a few days of dev-only query history still isn't a call to make casually) |
`docs/testing/EXECUTION-2026-09-08.md` finding 3's own verification; migrations
`20260909020000`/`20260909030000`/`20260909040000`/`20260909050000`-`20260909050400` |
...
| FSM ↔ inventory (F-14) | ~~`cancelJob()` never releases stock `reserveJobParts()` reserved on scheduling~~ — **Fixed 2026-09-09**: new `releaseJobParts()` mirrors `consumeJobParts()`'s own release step, called from `cancelJob()`. Live-verified the exact RPC mechanism against dev (rolled-back transaction). **Follow-up investigated 2026-09-09, still open -- structural, not a quick fix:** `cancelJob()`/`releaseJobParts()`/`listJobPartLines()` all call `createClient()` from `@cofounderai/core/db/server`, which is Next.js-request-scoped (reads the session from cookies) -- confirmed via grep that literally no test anywhere in this repo (`test:db`'s raw-`psql` scripts, or any existing `vitest` unit test) invokes that code path, only `service_role`/direct-SQL access. Hand-writing the same RPC call sequence in a `psql` test would just re-assert `adjust_stock_for_contract`'s already-tested `unreserve` behavior (`test-inventory-procedural.mjs`) with fancier fixtures around it -- it wouldn't actually exercise `listJobPartLines()`'s own resolution logic (job → opportunity → approved estimate → lines filtered to `kind='good'` items), which is the actual thing this item wants covered. Closing this for real needs a new kind of test harness (e.g. one that builds a service-role Supabase client and calls the real exported TS functions instead of `psql`), which is a bigger, standalone infrastructure investment, not proportionate to add for one behavior -- flagged here rather than padded with a test that would pass without testing the thing it claims to | `docs/testing/EXECUTION-2026-09-08.md` finding 2, from `TC-FSM-011` |
...
| Licensing enforcement (architecture doc) | **Rollout substantially completed 2026-09-09**: `requireModule()`/`hasModuleWrite()` (`packages/core/src/licensing/queries.ts`) are now called as the first line of ~70 write functions across all 33 remaining `mutations.ts` files in fsm/inventory/crm/gst (on top of the original one-per-module demonstration). Verified with `typecheck`/`build`/`lint`/`lint:boundaries` all green and the full `test:db` suite (467+ checks, exit 0) unaffected. **Deliberately excluded, not overlooked:** (1) any function reachable from a cron-drained domain-event consumer or a webhook route with no signed-in Supabase session — `requireModule()` goes through the request-scoped, cookie-based `createClient()`, and `core.has_module_write()`/`has_module()` are both revoked from the `anon` role, so calling it on such a path would throw on every invocation rather than gracefully degrade. Found one real instance of this risk by tracing call sites rather than applying the mechanical rule blindly: `module-gst/lib/einvoicing/mutations.ts#generateEinvoice`/`cancelEinvoice` are called directly (bypassing `contract/index.ts`'s own `hasModule()` pre-check) from the `document.issued` event consumer drained by `/api/cron/drain-events`, which authenticates via a bearer secret, not a user session — left both alone, documented inline in that file, and only added `requireModule()` to that file's third function (`upsertEinvoiceCredentials`, always interactive). Same reasoning already correctly excluded `module-fsm/lib/messages/mutations.ts#ingestInboundJobReply` (the `email-inbound` webhook path) and `module-fsm/lib/estimates/mutations.ts#{markEstimateViewed,approveEstimateByToken,declineEstimateByToken}`/`module-fsm/lib/customer-center/mutations.ts#{approveEstimateFromCenter,declineEstimateFromCenter}` (public portal-token paths, no `businessId` param at all — these already run entirely on `service_role` admin clients keyed by the token itself, not a licensing check). (2) id-only setters/status-toggle RPC wrappers with no `businessId` parameter on their signature at all, where adding one would be a call-signature change across every caller, not a one-line addition — tracked as its own follow-up if ever wanted: `module-crm/{channels,routing-rules}` `set*Active`, `module-fsm/attachments#deleteJobAttachment`, `module-fsm/estimates#reorderChargeLines`, `module-fsm/invoices#markInvoiceViewed`, `module-fsm/reminders#sendDueReminders` (whole file — also cron-only, same reasoning as (1)), `module-inventory/alerts` (whole file), `module-inventory/customers#{updateCustomer,setCustomerActive}`, `module-inventory/purchase-orders#{setPurchaseOrderStatus,receivePurchaseOrderItem}`, `module-inventory/sales-invoices` (whole file), `module-inventory/sales-orders#{setSalesOrderStatus,confirmSalesOrder,shipSalesOrder,cancelSalesOrder}`, `module-inventory/sales-returns#{approveSalesReturn,setSalesReturnStatus}`, `module-inventory/stock-transfers#{setStockTransferStatus,shipStockTransfer,cancelStockTransfer,receiveStockTransferItem}`, `module-inventory/{suppliers,warehouses}` `update*`/`set*Active`. RLS (`tenant AND licensed`) remains the authoritative backstop for every one of these regardless — this whole row is about the defense-in-depth layer only | `docs/testing/EXECUTION-2026-09-08.md` finding 4, from `TC-CORE-001` |
