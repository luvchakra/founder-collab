...
| Licensing (ADR-9) | `expireGracePeriods()` exists and is correct but nothing calls it — no cron route, not in `vercel.json` — so a cancelled license's grace period never actually expires to `denied` on its own | `docs/testing/EXECUTION-2026-09-08.md` finding 1, from `TC-CORE-003` |
