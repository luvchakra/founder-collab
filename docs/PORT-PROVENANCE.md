...
**Remaining for `SP-7`** (see the backlog row's own suggested split, tracked as sub-tasks in this session): Products/Customers/Suppliers/Team (remaining config/master-data routes); Inventory(stock)/Stock Transfers/Sales Orders/Sales Invoices/Sales Returns/Purchase Orders (transactional routes); GST Filing + einvoice/eway-bill panels and actions (GST routes); Admin/Audit Log/Alerts/Dashboard/Onboarding + `public_api_v1` promotion to `core` (admin/api routes). `SP-9` (manifest + `contract/index.ts`) still not started.
