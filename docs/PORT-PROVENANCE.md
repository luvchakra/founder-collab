...
**Remaining after this batch:** `public_api_v1` (stockpilot-ai-ops's `src/lib/api-v1/` -- API-key auth, generic CRUD, OpenAPI generation, 8 resource route handlers, ~1500 lines total) promoted to `core`, and `SP-9` (`module-inventory`'s own `manifest.ts`/`contract/index.ts`). Both substantial enough, and `public_api_v1` security-sensitive enough (a new external-facing auth mechanism), to warrant their own focused story rather than a rushed tail-end addition to this batch.
