...
| `apps/web/app/(dashboard)/dashboard/businesses/[businessId]/inventory/products/{page.tsx,actions.ts}`, `packages/module-inventory/src/components/products/{product-modal,products-list}.tsx` | stockpilot-ai-ops | `src/routes/_authenticated/products.tsx` (component body) | `9cee43a450994dd73e63f74992d060af4c123d65` | Same UI adaptation as warehouses (own shell, hand-rolled modal, no mobile card layout). **Deferred, not ported this slice:** the "Generate Barcode/QR" button + `BarcodeLabelDialog`, and the `ProductImportDialog` spreadsheet-import flow (`src/lib/{product-import,spreadsheet,barcode-scan}.ts`, `src/components/{product-import-dialog,barcode-image,barcode-label-dialog,camera-scan-dialog,qr-code,scan-input}.tsx`) — core CRUD is the master-data route's own job; barcode/import are additive features, same proportion of scope as dropping warehouses' mobile card layout. |
...
**Remaining for `SP-7`** (see the backlog row's own suggested split, tracked as sub-tasks in this session): Customers/Suppliers/Team (remaining config/master-data routes); Inventory(stock)/Stock Transfers/Sales Orders/Sales Invoices/Sales Returns/Purchase Orders (transactional routes); GST Filing + einvoice/eway-bill panels and actions (GST routes); Admin/Audit Log/Alerts/Dashboard/Onboarding + `public_api_v1` promotion to `core` (admin/api routes); the deferred barcode/import features noted above. `SP-9` (manifest + `contract/index.ts`) still not started.
