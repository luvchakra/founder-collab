...
> **Implementation status (recorded 2026-09-17, while building out QA automation).** All four layers are now built and tested.
>
> - **Layer 1**: `tenant AND licensed` policies across `core`/`discovery`/`inventory`, proven end to end by `scripts/test-core-license-lifecycle.mjs` (the full ADR-9 lifecycle: grace is read-only, expired denies but retains, reactivation restores) plus the per-schema RLS scripts.
> - **Layer 2**: the `proxy.ts` route guard — `updateSession` in `packages/core/src/db/middleware.ts`, tested in `update-session.test.ts`.
> - **Layer 3**: `requireModule(businessId, moduleKey)` in `packages/core/src/licensing/require-module.ts`. Defaults to `core.has_module_write()` so ADR-9's read-only grace window blocks a mutating action; `hasModule()` beside it is the non-throwing form for ADR-10's degraded-mode callers. **Not yet called from anywhere** — there are no mutating server actions in a licensed module today (inventory is still DB-only), so its call sites arrive with SP-7/F-1.
> - **Layer 4**: `buildNavModules()` in `packages/core/src/licensing/nav-modules.ts`, wired through `apps/web/app/(dashboard)/layout.tsx` (one query loads every business's entitlements) and `DashboardChrome` (picks the active business out of the URL). An unlicensed module renders as an upsell pointing at the licences page — never at a route layer 2 would 404 — per this section's own wording. With no business selected, every module links normally: "not licensed" is not a meaningful claim until a business is active.
