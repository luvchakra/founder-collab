 ...
 * succeeding.
 */
export async function POST(request: Request, { params }: { params: Promise<{ businessSlug: string; productId: string }> }) {
...
export async function GET(_request: Request, { params }: { params: Promise<{ businessSlug: string; productId: string }> }) {
  const { businessSlug, productId } = await params;
  const businessId = await resolveBusinessIdBySlug(businessSlug);
  if (!businessId) {
    return NextResponse.json({ ok: false, error: "Business not found." }, { status: 404 });
