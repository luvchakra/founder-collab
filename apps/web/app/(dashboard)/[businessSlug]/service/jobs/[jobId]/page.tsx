...
import {
  getJob,
  getJobContext,
  getJobOriginatingCrmOpportunityId,
  getRevisitJobLinks,
  jobHasInvoice,
  listJobAuditLog,
  listRecommendedPartsWithAvailability,
} from "@cofounderai/module-fsm/lib/jobs/queries";
...
export default async function JobDetailPage({ params }: { params: Promise<{ businessSlug: string; jobId: string }> }) {
  ...
  const [
    ...
    getJobOriginatingCrmOpportunityId(job),
  ]);
  ...
  return (
    <JobDetail
      ...
      originatingCrmOpportunityId={originatingCrmOpportunityId}
    />
  );
}
