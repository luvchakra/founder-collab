...
/**
 * Moved out from underneath `/inventory/` (this session -- same treatment `admin/api-keys`
 * already got: see that page's own doc comment): membership and role/permission data
 * span every module, not just inventory's, so nesting this under one module's own route
 * implied an ownership that never matched the data model. `getBusiness`/`listBusinessMembers`
 * still come from `module-inventory`'s tenancy queries (unmoved to keep this change
 * scoped to the route) -- `apps/*` is exempt from the module-internals-only import rule
 * (lint-import-boundaries.mjs), same as `admin/api-keys` already relies on for its own
 * `module-discovery` import. Ported originally from stockpilot-ai-ops's
 * routes/_authenticated/team.tsx -- still read-only: permissions are set by role, not per
 * person, until custom roles ship (matches the original's own scope exactly).
 */
