...
async function invoicesPath(businessId: string) {
  return `${await businessPath(businessId)}/inventory/sales-invoices`;
}
...
export async function generateSalesInvoiceAction(businessId: string, salesOrderId: string): Promise<string> {
  ...
  revalidatePath(await invoicesPath(businessId));
  return invoiceId;
}
