...
export default async function OpportunityDetailPage({
  ...
  params: Promise<{ businessSlug: string; opportunityId: string }>;
}) {
  const { businessSlug, opportunityId } = await params;
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6">
      <div>
        <Link href={`/${businessSlug}/crm/opportunities`} className="text-sm text-muted-foreground hover:underline">
          &larr; Back to opportunities
        </Link>
        <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-xl font-semibold">
              <Link href={`/${businessSlug}/crm/customers/${opportunity.party_id}`} className="hover:underline">
