...
async function opportunityPath(businessId: string, opportunityId: string) {
  return `${await businessPath(businessId)}/crm/opportunities/${opportunityId}`;
}
...
export async function addOpportunityProductAction(businessId: string, opportunityId: string, formData: FormData): Promise<void> {
  ...
  revalidatePath(await opportunityPath(businessId, opportunityId));
}
