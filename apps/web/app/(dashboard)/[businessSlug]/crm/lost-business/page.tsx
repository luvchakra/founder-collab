 ...
 */
export default async function CrmLostBusinessPage({ params }: { params: Promise<{ businessSlug: string }> }) {
            ...
            {queue.map((row) => (
              <li key={row.interactionId} className="flex flex-col gap-2 p-3 text-sm">
                <div className="flex min-w-0 items-start justify-between gap-2">
                  <Link href={`/${businessSlug}/crm/customers/${row.partyId ?? ""}`} className="min-w-0 font-medium break-words hover:underline">
              ...
              {queue.map((row) => (
                <TableRow key={row.interactionId}>
                  ...
                  <TableCell className="font-medium">
                    <Link href={`/${businessSlug}/crm/customers/${row.partyId ?? ""}`} className="hover:underline">
                      {row.partyName ?? "Unknown contact"}
                    </Link>
                  </TableCell>
                  <TableCell>
