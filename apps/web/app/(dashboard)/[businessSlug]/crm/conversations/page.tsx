...
function conversationHref(businessSlug: string, params: Record<string, string | undefined>) {
  const search = new URLSearchParams();
  for (const [key, value] of Object.entries(params)) {
    if (value) search.set(key, value);
  }
  const query = search.toString();
  return `/${businessSlug}/crm/conversations${query ? `?${query}` : ""}`;
}
      ...
      <div className="flex flex-wrap gap-2">
        {QUICK_FILTERS.map((f) => (
          <Button key={f.key} asChild size="sm" variant={isQuickFilterOn(f.key) ? "secondary" : "outline"}>
            <Link href={conversationHref(businessSlug, { ...activeFilterParams, [f.key]: isQuickFilterOn(f.key) ? undefined : "1" })}>{f.label}</Link>
          </Button>
        ))}
      </div>
    ...
    <div className="flex flex-col divide-y rounded-2xl border border-border">
      {rows.length === 0 ? (
        ...
        rows.map((row) => (
          <Link
            key={row.id}
            href={conversationHref(businessSlug, { ...activeFilterParams, ...Object.fromEntries(QUICK_FILTERS.filter((f) => isQuickFilterOn(f.key)).map((f) => [f.key, "1"])), conversationId: row.id })}
    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <CardTitle className="text-base">
            {selectedParty ? (
              <Link href={`/${businessSlug}/crm/customers/${selectedParty.id}`} className="hover:underline">
                {selectedParty.name}
              </Link>
            ) : (
              "Unknown contact"
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
          ...
          {selectedOpportunity ? (
            <Link href={`/${businessSlug}/crm/opportunities/${selectedOpportunity.id}`} className="hover:underline">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-[240px_minmax(0,1fr)_minmax(0,1.3fr)]">
        ...
        <div className={isViewingConversation ? "" : "hidden md:block"}>
          {isViewingConversation ? (
            <Link href={conversationHref(businessSlug, activeFilterParams)} className="mb-3 block text-sm text-muted-foreground hover:underline md:hidden">
