...
async function main() {
  await withTestDatabase({
    ...
    testFn: async ({ psql, psqlAs, assertEqual, assertThrows }) => {
      ...
      assertEqual(psqlAsBob("select count(*) from core.permissions"), "34", "the permission catalogue is readable by any authenticated user (23 from C-7 + 5 stock_transfers.* from SP-3b + 4 sales_returns.* from the sales-returns workflow migration + 1 opportunities.edit from F-2 + 1 estimates.edit from F-3)");
